...
#[cfg(feature = "timezones")]
fn convert_to_naive_local<T1: TimeZone, T2: TimeZone>(from_tz: &T1, to_tz: &T2, ndt: NaiveDateTime) -> NaiveDateTime {
    from_tz
...
#[cfg(feature = "timezones")]
fn convert_to_timestamp<T1: TimeZone, T2: TimeZone>(from_tz: T1, to_tz: T2, arr: &PrimitiveArray<i64>, tu: TimeUnit) -> ArrayRef{
    match tu {
...
pub fn cast_timezone(
    ...
    match from.parse::<chrono_tz::Tz>() {
        Ok(from_tz) => match to.parse::<chrono_tz::Tz>() {
            Ok(to_tz) => {
                convert_to_timestamp(from_tz, to_tz, arr, tu)
            }
