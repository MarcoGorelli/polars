...
#[pyfunction]
#[pyo3(signature = (start, end, interval="1d", periods=None, closed=Wrap(ClosedWindow::Both)))]
pub fn date_range(
    start: PyExpr,
    end: Option<PyExpr>,
    interval: &str,
    ...
    let closed = closed.0;
    dsl::date_range(start, end, interval, periods, closed).into()
}
...
pub fn date_ranges(
    ...
    let interval = Duration::parse(interval);
    let closed = closed.0;
    dsl::date_ranges(start, end, interval, periods, closed).into()
}

#[pyfunction]
#[pyo3(signature = (start, end, interval="1d", periods=None, closed=Wrap(ClosedWindow::Both), time_unit=None, time_zone=None))]
pub fn datetime_range(
    ...
    dsl::datetime_range(start, end, interval, periods, closed, time_unit, time_zone).into()
}
...
pub fn datetime_ranges(
    ...
    let closed = closed.0;
    let time_unit = time_unit.map(|x| x.0);
    dsl::datetime_ranges(start, end, interval, periods, closed, time_unit, time_zone).into()
}
