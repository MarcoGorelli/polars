...
pub trait SeriesOpsTime: AsSeries {
    ...
    /// Apply a time-based rolling mean to a Series.
    ///
    /// See: [`RollingAgg::rolling_mean_by`]
    #[cfg(feature = "rolling_window")]
    fn rolling_mean_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().to_float()?;
        with_match_physical_float_polars_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            rolling_agg_by(
    ...
    /// Apply a rolling sum to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_sum_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().clone();
    ...
    /// Apply a rolling quantile to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_quantile_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().to_float()?;
        with_match_physical_float_polars_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
        rolling_agg_by(
    ...
    /// Apply a rolling min to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_min_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().clone();
    ...
    /// Apply a rolling max to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_max_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().clone();
        with_match_physical_numeric_polars_type!(s.dtype(), |$T| {
            let ca: &ChunkedArray<$T> = s.as_ref().as_ref().as_ref();
            rolling_agg_by(
    ...
    /// Apply a rolling variance to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_var_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        let s = self.as_series().to_float()?;
    ...
    /// Apply a rolling std_dev to a Series.
    #[cfg(feature = "rolling_window")]
    fn rolling_std_by(&self, options: RollingOptionsImpl) -> PolarsResult<Series> {
        self.rolling_var_by(options).map(|mut s| {
            match s.dtype().clone() {
                DataType::Float32 => {
                    let ca: &mut ChunkedArray<Float32Type> = s._get_inner_mut().as_mut();
                    ca.apply_mut(|v| v.powf(0.5))
