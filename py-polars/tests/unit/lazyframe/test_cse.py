...
def test_cse_custom_io_source_same_object() -> None:
    ...
    caches: list[str] = [
        x
        for x in map(
            str.strip, plan.splitlines()
        )  # pyrefly: ignore[bad-argument-type] (todo)
    ...
    caches = [
        x
        for x in map(str.strip, plan.splitlines())
        if x.startswith("CACHE[")  # pyrefly: ignore[bad-argument-type] (todo)
    ]
    assert len(caches) == 0
